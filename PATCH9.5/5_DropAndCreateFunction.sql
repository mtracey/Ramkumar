...
IF EXISTS ( SELECT  *
            FROM    sys.objects
            WHERE   object_id = OBJECT_ID(N'convettohoursfromdecmial') AND
                    type IN (N'FN', N'IF', N'TF', N'FS', N'FT') ) 
   DROP FUNCTION convettohoursfromdecmial
   

IF EXISTS ( SELECT  *
            FROM    sys.objects
            WHERE   object_id = OBJECT_ID(N'fn_Split') AND
                    type IN (N'FN', N'IF', N'TF', N'FS', N'FT') ) 
...
IF EXISTS ( SELECT  *
            FROM    sys.objects
            WHERE   object_id = OBJECT_ID(N'Rounding') AND
                    type IN (N'FN', N'IF', N'TF', N'FS', N'FT') ) 
   DROP FUNCTION Rounding
...
----CPF 2011
----SELECT [dbo].[fn_getSDLAmount1] (2500,9,2011)
---CPF Changes
--SET QUOTED_IDENTIFIER ON
--GO
--CREATE  FUNCTION [dbo].[fn_getSDLAmount] (@fundgrossamount INT,@month INT, @year INT)
--RETURNS FLOAT
--AS 
--    BEGIN 
...
--        FROM    (SELECT SDL = CASE WHEN (@fundgrossamount > 0 AND
--                                         --@fundgrossamount <= 4500 
--										@fundgrossamount <= @cpfceil			
--                                        ) THEN (@fundgrossamount * 0.0025)
--                                   -- WHEN (@fundgrossamount >= 4500.00000001) THEN (4500 * 0.0025)-- (@fundgrossamount >= @factor) THEN (@cpfceil * 0.0025)
--									WHEN (@fundgrossamount >= @factor) THEN (@cpfceil * 0.0025)
--                                   WHEN (@fundgrossamount <= 0) THEN 0
--                              END
--                ) D
--        RETURN @sdlamt
...
AS
    BEGIN
        ...
        SET @cpfceil = 4500
        SET @layear = @cuyear - 1
        SET @cpfawcil = 76500 --additional wages
        --additional wages    Get Additional wages based up on the employee worked period
        ---Example for 2011 before 1 sept $76500
        --1sept 2011 to 1 dec 2011 79333
        ---From 1 Jan 2012 $85000 
        SET @AWCIL = 0
...
--Set @cuyear  = 2008      
        ...
        --	CPF Ceiling overwrite as declared by CPF board.(All rates come from table Company_CPF_CEILING and not from company table)
			...
			SELECT @cpfceil=monthly_cpf_celi, @cpfawcil =annual_cpf_ceil from Company_CPF_CEILING where convert(datetime,@PayrollDate,103) between 
        	convert(datetime,EffectiveDateFrom,103) and convert(datetime,EffectiveDateTo,103)
...
BEGIN                  
      DECLARE @startsubpay VARCHAR(10)                                                                                                                
       ...
       DECLARE @layear INT    ----Last Year                      
       DECLARE @AWCIL FLOAT                ---Add Wages Ci          
       DECLARE @cpfawcil FLOAT              ---CPD aw cil        
       DECLARE @LYOW FLOAT                  ---Last year OW        
               
       DECLARE @CYOW FLOAT                  --Current year ow        
       DECLARE @CMOW FLOAT                  ---current month ow        
       DECLARE @cpfceil FLOAT               -----cpfceil        
       DECLARE @AWCM FLOAT                  ---add wages current month        
       DECLARE @AWB4CM FLOAT                  ---awages before curremt month        
       DECLARE @retval FLOAT                  ----retval        
       DECLARE @monthid INT                  --month        
       DECLARE @awinloop FLOAT                          
       DECLARE @awinloopsum FLOAT                          
       DECLARE @leftcumonth INT                          
       DECLARE @ACTCIL FLOAT                 -----Ceiling -ord cpf        
       DECLARE @lastdaymonth VARCHAR(2)                                                                                                
       declare @RowNumber int               
                     
                             
       DECLARE @OW_OLDRATECY FLOAT      ---        
       DECLARE @OW_NEWRATECY FLOAT                
       DECLARE @CPFFACTOR FLOAT              
       DECLARE @MONTH_CHANGED INT              
       DECLARE @MONTHTEMP INT              
       DECLARE @BASICCM FLOAT              
       DECLARE @NEWFLAG VARCHAR(20)              
       DECLARE @AWSCPFCYTD FLOAT        
               
       DECLARE @CMOWD FLOAT   --For deductions           
    DECLARE @LYOW1 FLOAT            
       ----NEW VAR          
       DECLARE @CYOWTLM FLOAT          
          
       DECLARE @cpfawcil_ly FLOAT ---Last year AW Ceiling          
       ...
       Declare @PayrollDate varchar(200)           
       DECLARE @PayrollDate_1 varchar(200)   --Last year AW Ceiling              
                          
       SET @PayrollDate ='01/' + cast(@month as varchar(20)) + '/' + cast(@cuyear as varchar(20))                
       SET @PayrollDate_1 ='01/' + cast(@month as varchar(20)) + '/' + cast(@layear as varchar(20))            
                  
                    
     Select @cpfceil=monthly_cpf_celi              
     from Company_CPF_CEILING               
     where convert(datetime,@PayrollDate,103)               
     between  convert(datetime,EffectiveDateFrom,103) and convert(datetime,EffectiveDateTo,103)                   
            
  ---Print @cpfceil    
      
  ---print '*************************************************'    
    
     Select @cpfawcil=Additional_Wage_Ceiling               
     from CPF_AdditionalWages_CEILING               
     where convert(datetime,@PayrollDate,103)               
     between   convert(datetime,EffectiveDateFrom,103) and convert(datetime,EffectiveDateTo,103)                 
                  
        ---Get Last year AW ceiling          
        Select @cpfawcil_ly=Additional_Wage_Ceiling               
     from CPF_AdditionalWages_CEILING               
     where convert(datetime,@PayrollDate_1,103)               
     between   convert(datetime,EffectiveDateFrom,103) and convert(datetime,EffectiveDateTo,103)            
 ...
 GROUP BY Emp_ID                    
    ...
    ---SET @LYOW=56000         
                SELECT  @LYOW1= ISNULL(LYTotalOW, 0)             
                FROM   Employee                      
                WHERE  Emp_Code = @empid AND                      
                OWLastYear = (@layear)         
     ...
     END     
         ...
         SET  @AWCM =0              
          ...
          IF (@LYOW >0 AND  @month >=9 AND @cuyear=2011 )                    
                    
                BEGIN                             
                         SET @NEWFLAG='YES'              
                               
                             SELECT @OW_OLDRATECY = S.OrdinaryWagesCY FROM               
                              (              
                              SELECT  SUM(T.OrdinaryWagesCY) OrdinaryWagesCY                    
                                FROM (SELECT Emp_ID ,                    
                                             OrdinaryWagesCY = CASE WHEN OrdinaryWagesCY >= @cpfceil THEN @cpfceil                    
                                                                    ELSE ISNULL(OrdinaryWagesCY, 0)                    
                                                               END                    
                         ...
                         INNER JOIN prepare_payroll_detail pd ON ph.trx_id = pd.trx_id                    
                          ...
                          IF(@cpfceil <=@basicascmow)              
                           BEGIN               
                            SET @CPFFACTOR =  @cpfceil               
                           END               
                          ELSE              
                           BEGIN               
                            SET @CPFFACTOR  =@basicascmow              
                                         
                           END                                   
                                        
                          SET @MONTH_CHANGED =@month                                    
           SET @MONTHTEMP =0              
                                        
                          SELECT @MONTHTEMP = MONTH(termination_date)                      
                        FROM     Employee              
                        WHERE    Emp_Code = @empid AND                      
                          MONTH(termination_date) < 12              
                                                                   
                          IF (@MONTHTEMP >0 AND @month=@MONTHTEMP)              
                            BEGIN              
                               SET @OW_NEWRATECY= @CPFFACTOR                                 
            END               
                          ELSE                                  
                            BEGIN              
                              SET @OW_NEWRATECY= @CPFFACTOR *(12-(@month-1))              
                            END                 
                                      
                            IF (@month =@MONTHTEMP)          
                             BEGIN          
                                                           
                                SET @LYOW= @OW_OLDRATECY+ @OW_NEWRATECY            
       ...
       FROM  (          
         ...
         ---SELECT * FROM prepare_payroll_detail pd WHERE pd.emp_id =18       
    
                 ----print @cpfceil     
                           
         SELECT @CMOW = ISNULL(R.AWPaidCYMonth, 0)   ---CASE WHEN ISNULL(R.AWPaidCYMonth, 0) >= @cpfceil THEN @cpfceil                    
                             --ELSE ISNULL(R.AWPaidCYMonth, 0)                    
                        --END                    
         FROM     (SELECT R.Emp_Code, SUM(ISNULL(R.AWPaidCYMonth,0))+(ISNULL(@basicascmow,0) -ISNULL(@UnpaidDeduct,0)) AWPaidCYMonth                    
                 FROM   (SELECT Emp_Code, MONTH(trx_period) [Mth], SUM(trx_amount) AWPaidCYMonth                    
                         FROM   emp_additions EA                    
                         INNER JOIN Additions_types AT ON EA.Trx_Type = AT.ID                    
                         WHERE                                   (MONTH(trx_period) = @month AND                    
      ...
      SET @CMOW = ISNULL(@CMOW, (@basicascmow -@UnpaidDeduct))---+(@basicascmow -@UnpaidDeduct))      
          
               IF ISNULL(@CMOW, 0) >= @cpfceil      
     BEGIN    
      SET @CMOW =@cpfceil     
     END    
     --Changes for CMOW ---For All clients       
...
--                ) R      
     ...
     SELECT   @AWB4CM = R.AWPaidCY                    
               FROM     (SELECT R.Emp_Code, SUM(R.AWPaidCY) AWPaidCY                    
                 FROM   (SELECT EA.Emp_Code, MONTH(trx_period) [Mth], SUM(trx_amount) AWPaidCY                    
                         FROM   emp_additions EA                    
                         INNER JOIN Additions_types AT ON EA.Trx_Type = AT.ID    
        ...
        AT.Type_Of_Wage = 'A' AND                    
                                EA.Emp_Code = @empid      
        AND     
        ( (( e.emp_type = 'SPR' OR e.emp_type = 'SDPR' ) AND (MONTH(trx_period) >=MONTH(E.pr_date) ))    
         OR     
         e.emp_type='SC'     
        )      
                         GROUP BY EA.Emp_Code, MONTH(trx_period)                    
                        ) AS R                    
                 GROUP BY R.Emp_Code                    
                ) R         
...
--                               
--         BEGIN              
--            Select @BASICCM =  convert(varchar(10),DecryptByAsymKey(AsymKey_ID('AsymKey'), payrate))                
--           from employee WHERE emp_code =@empid              
--                                                    
--          INSERT @CPFAWCALC              
--            (Emp_Code,CMOW,LYOW,CYOW,CPFAWCIL,EST_AWCIL,ACTCIL,AWCM,AWB4CM,AWCM_AWB4CM,AWCPF)                                        
--          SELECT *              
--          FROM   dbo.Fn_calculatecpfaw (@company_id, @monthid, @cuyear, @empid, @BASICCM,@UnpaidDeduct)                
--                                          
--          END              
--                                    
--                                   
...
--                DEALLOCATE db_cursor                
    ...
    Declare @enddate VARCHAR(100)      
               ...
               SET @startdate = '01/01/'+ cast(@cuyear AS VARCHAR(100))    
    SET @enddate = '31/12/'+cast(@cuyear AS VARCHAR(100))    
        
          
        
     SELECT @AWSCPFCYTD= SUM(A.cpfAdd_Additional) FROM     
    (    
     SELECT  sum(cpfAdd_Additional) cpfAdd_Additional   from PayRollView      
     where emp_code in (@empid)     
     and Convert(Datetime,start_period,103) between Convert(Datetime,@startdate,103) And     
     Convert(Datetime,@enddate,103) and Convert(Datetime,end_period,103) between Convert(Datetime,@startdate,103)    
      And Convert(Datetime,@enddate,103) And STATUS in ('G')  and cpfnet!=0     
     GROUP BY  basic_pay, cpfAdd_Ordinary, cpfAdd_Additional, cpfAmount, cpfNet, NetPay, SDL, total_additions,     
     total_gross,EMP_NAME,EMP_lname,start_period,end_period       
    )A             
           
                       
               ---SELECT @AWSCPFCYTD = SUM(AWCPF) FROM @CPFAWCALC              
    ---SELECT @AWSCPFCYTD = SUM( CASE WHEN AWCPF >0 THEN AWCPF ELSE 0 END) FROM @CPFAWCALC            
                       
                ------PRINT @AWSCPFCYTD       
    if   @AWSCPFCYTD is null      
     BEGIN       
      SET @AWSCPFCYTD =0      
     END        
              ...
              BEGIN                   
                ...
                IF @month < 12                    
                   ...
                   IF @LYOW <= 0                                  
                                   ...
                                   ----PRINT '(((((((((((((((((((((((((((((((((('          
                                           ...
                                           ----PRINT 'XXX4'              
                                             IF (@retval > @AWCM)                     
                                              ...
                                              ----PRINT 'XXX5'                                                                                                           
                                                END                
                                             ELSE IF (@AWCM+@awinloopsum>=@cpfceil)                  
                                              BEGIN             
                                              ...
                                              BEGIN          
                                                 ...
                                                 ----PRINT @cpfawcil          
                                                  ...
                                                  ----PRINT @cpfawcil          
                                                   ...
                                                   SET @retval =@cpfawcil  -(@CYOWTLM + @AWSCPFCYTD +@cpfceil)          
                        
                                             END           
                             END               
                                       ELSE              
                                         BEGIN               
                                          SET @retval = @retval -@cpfceil              
                                           ----PRINT 'XXX7'          
                                         END                                        
                                         END            
                        ...
                        IF(@leftcumonth = @month)          
                                                              BEGIN        
                                                                    IF ((ISNULL(@CMOW, 0) + ISNULL(@basicascmow, 0)) -  ISNULL(@CMOWD, 0) ) >= @cpfceil                   
                                                   BEGIN                          
                                                       SET @basicascmow =@cpfceil                
                                                   END                          
                                                ELSE        
                                                    BEGIN                          
                                                       SET @basicascmow =(ISNULL(@CMOW, 0) + ISNULL(@basicascmow, 0) ) -@CMOWD                
                                                      END          
                     ...
                     IF (@ACTCIL <= @AWB4CM)                     
                                                     BEGIN         
                                                          ...
                                                          ----PRINT 'PPPPPPPPP00000000000'         
                                                        IF(@AWSCPFCYTD >=(@cpfawcil_ly-@LYOW))            
                                                                BEGIN         
                                                             --SET @retval = @ACTCIL - @AWB4CM              
                                                             SET @retval = @cpfawcil -(@CYOW+@AWSCPFCYTD) --Cganes for Testcase 3          
                                                             ----PRINT 'XXX13DDDDDDDDDDDD'                                               
                                                            END        
                                                           ELSE        
                                                            BEGIN        
                                                             SET @retval  =@AWCM        
                                                ...
                                                IF (@ACTCIL >= @AWB4CM)                     
                                                   BEGIN                 
                                                      ...
                                                      ----PRINT 'PPPPPPPPP00000000000111111111111'            
                                                         IF ((@ACTCIL - @AWSCPFCYTD) >= @AWCM)                     
                                                             BEGIN          
                                                              IF(@AWSCPFCYTD >=(@cpfawcil_ly-@LYOW))                  
                                                               BEGIN         
                                                                     SET @retval =  @ACTCIL - @AWSCPFCYTD                 
                                                                     ----PRINT 'XXX14FFFFFFFFFFFFF'           
                                                                END        
                                                                   ELSE        
                                                                    BEGIN         
                                                                     SET @retval =@AWCM        
                                                                      ----PRINT 'BBBBCFRFRFGRFR'          
                              ...
                              BEGIN         
                               ...
                               -- BEGIN                   
                                                                     ...
                                                                     ----PRINT @ACTCIL        
                                                                     ----PRINT @AWSCPFCYTD          
                                        ----PRINT 'XXXCCCCCCCCCCCCCCCBBBBBBBBBBBBBBBBBB'                                                                           
                                                             --    END        
                                                             -- ELSE        
                                                             --  BEGIN         
                                                                       
                                                             ---   SET @retval =@AWCM        
                                                            --     ----PRINT 'BBBBCFRFRFGRFRNNNNNNNNNNNNNNNN'          
                                                            ---   END         
                                                ...
                                                ELSE        
                                                 BEGIN         
                                                   SET @retval= @AWCM        
                                                   ----PRINT 'newvvvvvvvvvvvvvvvvvvvvFFFFFFFFFFFFFFFFFFFFFFFFF'          
                                                 END         
                                                              
                                           ----IF Annaulwage ceiling + OW Till date >CPFCeiling then refund the remaining amount          
                                                   ----PRINT @retval        
                                     ...
                                     --IF((@AWCIL+@CYOWTLM)>=@cpfawcil)          
                                                --    BEGIN          
                                                --     SET @retval =@CYOWTLM-@CYOW          
                                                 --    ----PRINT 'newXXXXXXXXXXXXXXXXXXXXX'          
                                                   ----- END                                                                               
...
--                                                                          ----PRINT @awinloopsum        
--                                                                                     
--                                                                            IF ((@cpfawcil_ly- @LYOW) = @AWSCPFCYTD)        
--                                                                             BEGIN                                                                   
--                                                                               SET @retval = @cpfawcil -@cpfawcil_ly         
--                                                                               ----PRINT @cpfawcil        
--                                                                               ----PRINT @cpfawcil_ly         
--                                                                                ----PRINT @retval           
--                                                                               ----PRINT 'XXX99999999'          
--                                                                             END          
--                                                                           ------Terminating current month          
--                                                                    END                          
                                                                   ...
                                                                   IF @ACTCIL > (@AWB4CM + @AWCM)              
                                                                       BEGIN                           
                                                                         --SET @retval = (@AWB4CM + @AWCM) - ISNULL(@awinloopsum, 0)            
                   SET @retval =  @AWCM ---- ISNULL(@awinloopsum, 0)            
                                                                             -- ----PRINT             
                                                                           ----PRINT 'XXX10'           
                   ...
                   IF @ACTCIL <= (@AWB4CM + @AWCM)           
                                                                 BEGIN     
                                                                     SET @retval = @ACTCIL -@AWB4CM ---+ @AWCM)  -- @AWCM    
                                                                  --PRINT 'ppppppppppppsdssssssssssssss'    
                                                                 END    
                                                                  END                  
                                                 ...
                                                 IF(@leftcumonth = @month)          
                                                   ...
                                                   BEGIN                          
                                                       SET @basicascmow =@cpfceil          
                                                        ----PRINT 'TCCCCCCCCCCCCCCCCCCC'                   
                                                   END                          
                                                ELSE        
                                                    BEGIN                          
                                                       SET @basicascmow =(ISNULL(@CMOW, 0) + ISNULL(@basicascmow, 0) ) -@CMOWD           
                                                         ----PRINT 'TCCCCCCCCCCCCCCCCCCCFFFFFFFFFFFFFF'             
                                                      END          
                                               ...
                                               BEGIN                     
                                                     ...
                                                     ----PRINT  @AWB4CM        
                                                     ----PRINT  @AWCM        
                                                     ----PRINT 'VVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVFFFFFFFFFFFFFff'                
       ...
       BEGIN          
                                                             ...
                                                             BEGIN          
                                                              ...
                                                              IF(@AWSCPFCYTD >=(@cpfawcil_ly-@LYOW))                  
                                                               BEGIN         
                                                                     ...
                                                                     IF(@ACTCIL<=(@AWB4CM+@AWCM))        
                                                                      BEGIN         
                                                                       ----PRINT 'XXX199999999999999999999'        
                                                                       SET @retval  =@ACTCIL -@AWSCPFCYTD        
                                                                      END         
                                                                     ELSE        
                                                                      BEGIN         
                                                                        ----PRINT 'XXX19999999999999999999999999999999'        
                                                                       SET @retval  =(@AWB4CM) -@AWSCPFCYTD        
                                                                      END         
                                    ...
                                    SET @retval =@AWCM        
                                                                     ...
                                                                     IF ((ISNULL(@ACTCIL, 0)-ISNULL(@AWSCPFCYTD,0))<=ISNULL(@AWB4CM,0))        
                                                                      BEGIN         
                                                                        ----PRINT 'V39999999999999999XXXXXXXXXXXXXXXXX'         
                                                                       SET @retval =ISNULL(@AWB4CM, 0) -ISNULL(@AWSCPFCYTD,0)        
                                                                      END         
                                                                              
                                                                     IF(@AWCM >0)        
                                                                      BEGIN         
                                                                             
                                                                       IF ((ISNULL(@ACTCIL, 0)-ISNULL(@AWSCPFCYTD,0)) >@AWCM)        
                                                                        BEGIN         
                                                                          SET @retval =ISNULL(@AWCM, 0)        
                                                                           ----PRINT 'V39999999999999999'        
                                                                        END         
                                                                       ELSE        
                                                                        BEGIN         
                                                                          SET @retval =(ISNULL(@ACTCIL, 0)-ISNULL(@AWSCPFCYTD,0))        
                                                                           ----PRINT 'V39999999999999999XXXXXXXXXXX'        
                                                                        END         
                                                               END         
                                                                    END         
                                                             END         
                                                                  ...
                                                                  BEGIN         
                                                                    ...
                                                                    END        
                                                                     ELSE        
                                                                       BEGIN        
                                                                        ----PRINT ';;;;;;;;;;;;;;;;;;;;;;'        
                                                                         SET @retval =  @ACTCIL -  ISNULL(@AWSCPFCYTD, 0)             
                                                                         ----PRINT @ACTCIL        
                                            ----PRINT @AWSCPFCYTD        
                                                                         ----PRINT  @retval        
                                           ----PRINT @cpfawcil_ly        
                                                                         ...
                                                                         IF (ISNULL(@retval, 0) >=(@AWCM))        
                                                                          BEGIN         
                                                                           SET @retval =@AWCM        
                                                                           ----PRINT 'MMMMMMMMMMMMMMMMMMMMMMLLLLLLLLLLLLLLLLLLL'        
                                                                          END          
...
BEGIN         
   ...
   BEGIN         
                                                ...
                                                ELSE        
                                                 BEGIN         
                                                   SET @retval= @AWCM        
                                                   ----PRINT 'newvvvvvvvvvvvvvvvvvvvvCCCCCCCCCCCCCCCCCCCCC'        
                                                                                                                                                     
                                                   IF ((@AWB4CM +@AWCM)  >(@cpfawcil_ly-@LYOW))        
                                                    BEGIN        
                                                       SET @retval = @ACTCIL - @AWSCPFCYTD        
                                                        ----PRINT '[[[[[[[[[[[[[[[[[[[[[[[[[[['         
                                                    END          
                                                 END         
                                                   ...
                                                   ----PRINT  @retval           
                                                   ----IF Annaulwage ceiling + OW Till date >CPFCeiling then refund the remaining amount          
                                                   ----PRINT '****************************************FFFSADDDDDDDDDDDDDDDDDDDDDDDDD'          
                                                   ----PRINT  @retval        
   ...
   BEGIN        
       ----PRINT 'MONTH 1222222222222222'        
         ...
         SET @ACTCIL = @cpfawcil - @CYOW          
                                   ...
                                   BEGIN         
                                     IF(@AWSCPFCYTD >=(@cpfawcil_ly-@LYOW))           
                                          BEGIN                     
                                           --SET @retval = @ACTCIL - @AWB4CM              
                                            --SET @retval = @cpfawcil -(@CYOW+@AWSCPFCYTD) --Cganes for Testcase 3          
            SET @retval = (@cpfawcil -56000)-@AWSCPFCYTD    
                                            ----PRINT 'XXX131222222222'                                               
                                          END         
                            ELSE        
                                          BEGIN         
                                            SET @retval =@AWCM        
                                   ...
                                   ----PRINT  @AWB4CM        
                                   ----PRINT  @AWCM        
                                   ----PRINT 'VVVVVVVVVVVVVVVVVVVVVVVVVVVVVV1222222222222222222'                
            ...
            ---SET @retval =0      
            SET @retval =0     
            IF  (@AWCIL-@AWB4CM >0) ---Comba Issue    
             BEGIN     
               SET @retval =@AWCIL-@AWB4CM     
             END    
                                             END         
         ...
         SET @retval = (@cpfawcil -56000)-@AWSCPFCYTD    
                                 ----PRINT @ACTCIL        
                                 ----PRINT @AWSCPFCYTD        
                                 ----PRINT 'newvvvvvvvvvvvvvvvvvvvv12'          
                               END         
...
GO
		CREATE Function [dbo].[fn_Salary](@emp_code int, @year int)              
		  ...
		  IF @cnt>0 
					IF @cnt1>0       
						Select @Salary = SUM(dh_e)  + SUM(NH_e )+ SUM (total_gross) -  SUM (TOTAL_ADDITIONS)+ Sum(E.GrossPay)/ @cnt  
						   FROM   
						   (  
							(  
							 --Select total_gross,TOTAL_ADDITIONS,Emp_id,NH_e ,dh_e 
							 select isnull(total_gross,0)total_gross,isnull(TOTAL_ADDITIONS,0)TOTAL_ADDITIONS,Emp_id,isnull(NH_e,0)NH_e ,isnull(dh_e,0)dh_e   
							 FROM  prepare_payroll_detail pd       
							 INNER JOIN prepare_payroll_hdr ph           
							 on pd.trx_id = ph.trx_id      
							 WHERE Emp_id = @emp_code AND status =  'G' AND  year(start_period) = @year and year(end_period)=@year  
					...
					ELSE				
							Select @Salary =SUM(dh_e)  + SUM (A.total_gross) -  SUM (A.TOTAL_ADDITIONS)+SUM(NH_e)  
							   FROM   
								(  
								 --Select total_gross,TOTAL_ADDITIONS,Emp_id,NH_e ,dh_e  
								 select isnull(total_gross,0)total_gross,isnull(TOTAL_ADDITIONS,0)TOTAL_ADDITIONS,Emp_id,isnull(NH_e,0)NH_e ,isnull(dh_e,0) dh_e  
...
BEGIN           
Declare @Salary float  
Select @Salary =  sum(isnull(dh_e,0))  + SUM (isnull(total_gross,0)) -  SUM (isnull(TOTAL_ADDITIONS,0)) from prepare_payroll_detail pd   
...
BEGIN           
	...
	declare @NheSalary float
	 ...
	 Select @NheSalary =sum(NH_e) FROM  prepare_payroll_detail pd       
	 INNER JOIN prepare_payroll_hdr ph
	 on pd.trx_id = ph.trx_id      
	 WHERE Emp_id = @emp_code AND status =  'G' 
		AND  year(start_period) =@year  
  ...
  OtherAllowance amount_type = 7 */  
        ...
        WHERE   Emp_code = @emp_code AND
                ...
                trx_type IN (SELECT id
                             FROM   additions_types
                             WHERE  tax_payable = 'Yes' AND
                                    --tax_payable_options IN (9, 10, 11, 12)) 
                                     tax_payable_options IN (9,  11, 12))   
...
SET QUOTED_IDENTIFIER ON
GO
CREATE FUNCTION convettohoursfromdecmial(@hour DECIMAL(10,2))  
RETURNS DECIMAL(10,2)  
BEGIN   
  
 DECLARE @NH DECIMAL(10,2)  
 ...
 Select @NH=case   
   ...
   WHEN   CAST ((@hour -cast(@hour As int)) AS DECIMAL(10,2))=0.6 THEN  cast(@hour As int)+1  
   WHEN   CAST ((@hour -cast(@hour As int)) AS DECIMAL(10,2))=0.60 THEN  cast(@hour As int)+1  
   ELSE @hour  
   END  
...
SET QUOTED_IDENTIFIER ON
GO
CREATE FUNCTION [fn_Split](@text VARCHAR(MAX)) 
RETURNS 
...
SET QUOTED_IDENTIFIER ON
GO
CREATE FUNCTION Rounding(@hour DECIMAL(10,2),@round int)    

RETURNS DECIMAL(10,2)    

  BEGIN  
    
     DECLARE @WH DECIMAL(10,2)  
     DECLARE @rounding  decimal (10,2)  
     DECLARE @ot2 decimal (10,2)  
	 ...
	 IF( @round=0) 	 
	   ...
	   IF( @round=30)      
	      BEGIN       
	               SET @rounding =30  
			       SET @ot2 =@hour    
			       SET @factor   = @rounding/2    
			      --- SELECT  ( @ot2 - Round(@ot2, 0) )*100    
			            
			            
			       SET @WH =     
			       CASE       
			          WHEN CAST(@rounding AS INT) = 0 THEN @ot2     
			                     
			          --WHEN ( @ot2 - Round(@ot2, 0) ) >= (CAST(( @rounding / 100 ) AS DECIMAL(18, 4))   +  CAST(( @factor / 100 ) AS DECIMAL(18, 4)))  
			          --THEN Cast ((Round(@ot2, 0)+1)AS DECIMAL(10,2))   
			                  
			          WHEN ( @ot2 - Round(@ot2, 0) ) >= CAST(( @rounding / 100 ) AS DECIMAL(18, 4))  ---AND   ( @ot2 - Round(@ot2, 0) ) < 0.45  
			          THEN  Cast (@ot2  AS INT)+ 0.30 --Cast ((Round(@ot2, 0)+1)AS DECIMAL(10,2))                 
			          -------------------------------------THEN Round(@ot2 , 0) + CAST(( @rounding / 100 ) AS   DECIMAL(18, 4))     
			          WHEN ( @ot2 - Round(@ot2, 0) )*100= @rounding THEN @ot2   
			          WHEN  abs( @ot2 - Round(@ot2, 0) ) <=0.30   
			          THEN  Cast (@ot2  AS INT)---Cast (@ot2  AS INT)+ 0.30 ---Cast (@ot2  AS INT)  
			          ELSE  Cast (@ot2  AS INT)+ 0.30   
	      ...
	      IF( @round=15)      
		      BEGIN       
				       ...
				       SET @WH =     
				       CASE       
				          WHEN CAST(@rounding AS INT) = 0 
				          THEN @ot2     
				          
				          WHEN ( @ot2 - (CAST (@ot2 AS int)) ) <= 0.14
				          THEN  Cast (@ot2 AS INT)			          
				          
				          WHEN ( @ot2 - (CAST (@ot2 AS int)) ) >= 0.14 AND  ( @ot2 - (CAST (@ot2 AS int)) ) <= 0.29
				          THEN  Cast (@ot2 AS INT) +0.15  
				          
				          WHEN ( @ot2 - (CAST (@ot2 AS int)) ) >= 0.30 AND  ( @ot2 - (CAST (@ot2 AS int)) ) <= 0.44
				          THEN  Cast (@ot2 AS INT) +0.30 
				          
				          WHEN ( @ot2 - (CAST (@ot2 AS int)) ) >= 0.45 AND  ( @ot2 - (CAST (@ot2 AS int)) ) <= 0.59
				          THEN  Cast (@ot2 AS INT) +0.45			          
		      ...
		      IF( @round=10)      
			      BEGIN       
					       ...
					       CASE       
					          ...
					          WHEN ( @ot2 - (CAST (@ot2 AS int)) ) <= 0.09
					          THEN  Cast (@ot2 AS INT)			          
					          
					          WHEN ( @ot2 - (CAST (@ot2 AS int)) ) >= 0.10 AND  ( @ot2 - (CAST (@ot2 AS int)) ) <= 0.19
					          THEN  Cast (@ot2 AS INT) +0.10  
					          
					          WHEN ( @ot2 - (CAST (@ot2 AS int)) ) >= 0.20 AND  ( @ot2 - (CAST (@ot2 AS int)) ) <= 0.29
					          THEN  Cast (@ot2 AS INT) +0.20 
					          
					          WHEN ( @ot2 - (CAST (@ot2 AS int)) ) >= 0.30 AND  ( @ot2 - (CAST (@ot2 AS int)) ) <= 0.39
					          THEN  Cast (@ot2 AS INT) +0.30			          
					          
					          WHEN ( @ot2 - (CAST (@ot2 AS int)) ) >= 0.40 AND  ( @ot2 - (CAST (@ot2 AS int)) ) <= 0.49
					          THEN  Cast (@ot2 AS INT) +0.40  
					          
					          WHEN ( @ot2 - (CAST (@ot2 AS int)))  >= 0.50 AND   (@ot2 - (CAST (@ot2 AS int))) <= 0.59
					          THEN  Cast (@ot2 AS INT) +0.50				          
		      ...
		      IF( @round=5)      
				      BEGIN       
						       ...
						       CASE       
						          ...
						          WHEN ( @ot2 - (CAST (@ot2 AS int)) ) <= 0.04
						          THEN  Cast (@ot2 AS INT)			          
						          
						          WHEN ( @ot2 - (CAST (@ot2 AS int)) ) >= 0.05 AND  ( @ot2 - (CAST (@ot2 AS int)) ) <= 0.09
						          THEN  Cast (@ot2 AS INT) +0.05  
						          
						          WHEN ( @ot2 - (CAST (@ot2 AS int)) ) >= 0.10 AND  ( @ot2 - (CAST (@ot2 AS int)) ) <= 0.14
						          THEN  Cast (@ot2 AS INT) +0.10 
						          
						          WHEN ( @ot2 - (CAST (@ot2 AS int)) ) >= 0.15 AND  ( @ot2 - (CAST (@ot2 AS int)) ) <= 0.19
						          THEN  Cast (@ot2 AS INT) +0.15			          
						          
						          WHEN ( @ot2 - (CAST (@ot2 AS int)) ) >= 0.20 AND  ( @ot2 - (CAST (@ot2 AS int)) ) <= 0.24
						          THEN  Cast (@ot2 AS INT) +0.20  
						          
						          WHEN ( @ot2 - (CAST (@ot2 AS int)))  >= 0.25 AND   (@ot2 - (CAST (@ot2 AS int))) <= 0.29
						          THEN  Cast (@ot2 AS INT) +0.25   
						           
						          WHEN ( @ot2 - (CAST (@ot2 AS int)))  >= 0.30 AND   (@ot2 - (CAST (@ot2 AS int))) <= 0.34
						          THEN  Cast (@ot2 AS INT) +0.30
						          
						          WHEN ( @ot2 - (CAST (@ot2 AS int)))  >= 0.34 AND   (@ot2 - (CAST (@ot2 AS int))) <= 0.39
						          THEN  Cast (@ot2 AS INT) +0.35
						          
						          WHEN ( @ot2 - (CAST (@ot2 AS int)))  >= 0.40 AND   (@ot2 - (CAST (@ot2 AS int))) <= 0.44
						          THEN  Cast (@ot2 AS INT) +0.40
						          
						          WHEN ( @ot2 - (CAST (@ot2 AS int)))  >= 0.45 AND   (@ot2 - (CAST (@ot2 AS int))) <= 0.49
						          THEN  Cast (@ot2 AS INT) +0.45
						          
						          WHEN ( @ot2 - (CAST (@ot2 AS int)))  >= 0.50 AND   (@ot2 - (CAST (@ot2 AS int))) <= 0.54
						          THEN  Cast (@ot2 AS INT) +0.50
						          
						          WHEN ( @ot2 - (CAST (@ot2 AS int)))  >= 0.55 AND   (@ot2 - (CAST (@ot2 AS int))) <= 0.59
						          THEN  Cast (@ot2 AS INT) +0.55
